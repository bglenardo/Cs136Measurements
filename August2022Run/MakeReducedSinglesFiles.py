...
for var in variables:
    with up.open(datadir + filename)['Xe136'] as tree:
        print('\t{}...'.format(var))
        raw_data[var] = tree.arrays([var],library='np')[var]
...
for i in range(4):
    ...
    tmp_ge_energy = np.copy( raw_data['scp_amplitude'][:,2*i] )
    calibrated_ge_energy = InverseQuadratic( tmp_ge_energy, *ge_ecal[i] )
    calibrated_ge_time = ( np.copy(raw_data['scp_time'][:,2*i]) - \
				np.copy(raw_data['scp_pickoff'][:,0]) ) * ns_per_ch -\
                            ge_tcal[i]

    ge_pileup = np.copy( raw_data['scp_pileup'][:,2*i] )
    ...
    subdict['timestamp']  = raw_data['timestamp'][:,2*i]
